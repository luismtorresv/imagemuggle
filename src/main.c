...
int main(int argc, char** argv){
    if(argc < 3){
        ...
        fprintf(stderr, "Note: PNG support requires stb headers and compile with -DUSE_STB -Ithird_party\n");
    }
    ...
    if(argc >= 2 && loadPNG(argv[1], &src, &w, &h, &c) != 0){
        fprintf(stderr, "Could not load %s. You can integrate your own I/O functions.\n", argv[1]);
        return 1;
    } else if(argc < 2){
        fprintf(stderr, "Continuing without loaded image (menu demonstration only)...\n");
        w=256; h=256; c=3;
    ...
    while(!exit_flag){
        ...
        if(op == 1){
            ...
            float ang; printf("Angle (degrees): "); scanf("%f", &ang);
            rotate_concurrente(src, dst.m, w,h,c, ang, 4);
            unsigned char*** tmp = src; src = dst.m; dst.m = tmp;
        } else if(op == 4){
            int nw, nh; printf("New width: "); scanf("%d",&nw); printf("New height: "); scanf("%d",&nh);
    ...
    if(argc >= 3){
        if(savePNG(argv[2], src, w,h,c) != 0){
            fprintf(stderr, "PNG not saved (missing stb or integrate your save function).\n");
        } else {
            printf("Saved to %s\n", argv[2]);
        }
    } else {
    ...
    // cleanup
    // Note: 'src' may point to different contiguous blocks due to swaps; for simplicity let OS free on exit.
    free_image3d(&dst);
    return 0;
}
